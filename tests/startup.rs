...
impl TestPrinter {
    fn new() -> Self {
        TestPrinter {
            lang: LanguageData::load(hangman::lang::Language::Global),
        }
    }
}
...
impl GameUI for TestPrinter {
    fn print_message(
        &mut self,
        _key: hangman::messages::MessageKey,
        _color: Option<Color>,
        _extras: Option<&str>,
        _bold: bool,
    ) -> std::io::Result<()> {
...
fn integration_headless_init_extracts_music() {
    ...
    let _gd = GameData::new_with_music_dir(printer, false, dir.path().to_str().unwrap())
        .expect("game data new");
